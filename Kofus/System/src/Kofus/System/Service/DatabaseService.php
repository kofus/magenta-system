...
class DatabaseService extends AbstractService
{
    public function dump($filename)
    {
        ...
        switch ($type) {
        	case 'mysql':
        		exec('mysqldump --skip-set-charset --skip-lock-tables --default-character-set=utf8 -r '.escapeshellarg($filename).' --user='.escapeshellarg($dbParams['user']).' --password='.escapeshellarg($dbParams['password']).' --host='.escapeshellarg($dbParams['host']).' --port='. ((int) $dbParams['port']) . ' '. escapeshellarg($dbParams['dbname']));
        		break;
